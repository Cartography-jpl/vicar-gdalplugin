...
if test "x$want_gdal" = "xyes"; then
        ...
        if test "$ac_gdal_path" != ""; then
            ...
            GDAL_LIBS="-L$ac_gdal_path/lib -L$ac_gdal_path/lib64 -R$ac_gdal_path/lib64 -lgdal"
            GDAL_CFLAGS="-I$ac_gdal_path/include"
            succeeded=yes
        else
	    if test "$V2OLB" != "" && test "$GDALLIB" != ""; then
		GDAL_PREFIX="$GDALLIB"
		GDAL_LIBS="-L$GDAL_PREFIX/lib -L$ac_gdal_path/lib64 -R$ac_gdal_path/lib64 -lgdal"
...
if test "$build_gdal" = "yes"; then
  ...
  AC_PROJ(required, $2, default_search)
  ...
  AM_CONDITIONAL([HAVE_PROJ], [false])
  ...
  AM_CONDITIONAL([BUILD_PROJ], [false])
